...
#include <fcntl.h>
                                                                                //IBM-iseries_bringup
#define IBM_ISERIES_NON_REDIRECT           0
#define IBM_ISERIES_REDIRECT_STANDARD_OUT  1
#define IBM_ISERIES_REDIRECT_OUT_TO_FILE   2
#define IBM_ISERIES_REDIRECT_STANDARD_ERR  3
#define IBM_ISERIES_REDIRECT_ERR_TO_FILE   4

#define IBM_ISERIES_REDIRECT_ERR_SELECT    -1
#define IBM_ISERIES_REDIRECT_ERR_WAIT      -2
#define IBM_ISERIES_REDIRECT_ERR_WRITE     -3
static void throwIOException(JNIEnv *env, int errnum, const char *defaultDetail);
...
/*
 * Set the file descriptor to be non-block
 * fd: file descriptor
 */
static void setFileNonblock(int fd)
{
	int flags;
...
/*
 * Read the child process data and write these data to STDOUT, STDERR, or file.
 * rdFd:     file descriptor used for read data
 * wrFd:     file descriptor used for write data
 * redirect: redirect type
 * Return value: 0: success
 *               IBM_ISERIES_REDIRECT_ERR_WRITE: write error
 */
static int readAndWriteData(int rdFd, int wrFd, int redirect) {

	int bytesRead;
	/*This buffer length need to be bigger than the length
	 * of PIPE used for communication between parent process and child process */
	char readBuf[2048] = {0};
	int rc = 0;

	if((bytesRead = read(rdFd, readBuf, sizeof(readBuf)-1)) > 0)
	{
		if (DEBUG_FLAG) {
            printf("readAndWriteData: read bytes: %d, fd: %d\n", bytesRead, rdFd);
	    }

		readBuf[bytesRead] = '\0';
		if ( redirect == IBM_ISERIES_REDIRECT_STANDARD_OUT ) {
			rc = write(STDOUT_FILENO, readBuf, bytesRead);
		}
		else if ( redirect == IBM_ISERIES_REDIRECT_STANDARD_ERR ) {
		    rc = write(STDERR_FILENO, readBuf, bytesRead);
	    }
		else {
			if (wrFd != -1) {
				rc = write(wrFd, readBuf, bytesRead);
			}
		}
	}
	...
	if (rc < 0) {
		if (DEBUG_FLAG) {
            printf("readAndWriteData: failed. rdFd = %d, wrFd = %d, redirect = %d, errno = %d\n", rdFd, wrFd, redirect, errno);
		}
		return IBM_ISERIES_REDIRECT_ERR_WRITE;
	}
...
/*
 * Read the output of child process in parent process through select() under IBM i.
 * outFd:	pipe file descriptor (for stdout) read by parent process
 * errFd:   pipe file descriptor (for stderr) read by parent process
 * fdout:   file descriptor (for output) wrote by parent process
 * fderr:   file descriptor (for error output) wrote by parent process
 * outRedirect: the type of output redirection
 * errRedirect: the type of error output redirection
 * pid:     PID of child process
 */
static int readChildProcessData(int outFd, int errFd, int fdout, int fderr, int outRedirect, int errRedirect, int pid)
{
	int bytes_read;
	int exit_status;
	int child_exited = 0;
	...
	/* This structure defines the base of the ILE arglist for calling _ILECALL. */
	typedef struct {
		 ILEarglist_base base;
		 int32  pid;
	} MethodArgs;

	/* Define the signatures for the arguments */
	...
	char buf2[sizeof(MethodArgs) + 16];
	MethodArgs* margs = (MethodArgs*) ROUND_QUAD(buf2);
	memset(margs,0x00,sizeof(MethodArgs));
	margs->pid = pid;

	FD_ZERO(&read_fds);

	if ( (outRedirect != IBM_ISERIES_NON_REDIRECT) && (errRedirect == IBM_ISERIES_NON_REDIRECT) ) {
		FD_SET(outFd, &read_fds);
		nfds = outFd + 1;
	} else if ( (outRedirect == IBM_ISERIES_NON_REDIRECT) && (errRedirect != IBM_ISERIES_NON_REDIRECT) ) {
		FD_SET(errFd, &read_fds);
		nfds = errFd + 1;
	} else if ( (outRedirect != IBM_ISERIES_NON_REDIRECT) && (errRedirect != IBM_ISERIES_NON_REDIRECT) ) {
		FD_SET(outFd, &read_fds);
		FD_SET(errFd, &read_fds);
		nfds = outFd > errFd ? outFd : errFd;
		nfds++;
	} else {
		debug_printf("readChildProcessData: outRedirect and errRedirect wrong value");
		return 0;
	}

	while(!child_exited)
	{
	    rc = select(nfds, &read_fds, NULL, NULL, NULL);
	    if(rc < 0) {
	    	debug_printf("readChildProcessData: select failed");
	        return IBM_ISERIES_REDIRECT_ERR_SELECT;
	    }

	    if(rc > 0) {
	        if(outRedirect&&(FD_ISSET(outFd, &read_fds))) {
	        	if (readAndWriteData(outFd, fdout, outRedirect) == IBM_ISERIES_REDIRECT_ERR_WRITE) {
	        		return IBM_ISERIES_REDIRECT_ERR_WRITE;
	        	}
	        }

	        if(errRedirect&&(FD_ISSET(errFd, &read_fds))) {
	        	if (readAndWriteData(errFd, fderr, errRedirect) == IBM_ISERIES_REDIRECT_ERR_WRITE ) {
	        		return IBM_ISERIES_REDIRECT_ERR_WRITE;
	        	}
	        }
	    }
	    ...
	    // Call ile
	    Xj9CallIleProcedure("Qyj9WaitForProcessExitWithNoHang",
	                   "QYJ9NTVUTL",
	                   NULL,
	                   XJ9_CALLILE_NO_JNIENV,
	                   (ILEarglist_base*)&(margs->base),
	                   signature,
	                   RESULT_INT32);

	    rc = margs->base.result.s_int32.r_int32;
	    if (rc == -1) {
	    	if (DEBUG_FLAG) {
	            printf("readChildProcessData: failure on waitpid, pid=%d, errno=%d\n", pid, errno);
	    	}
	        return IBM_ISERIES_REDIRECT_ERR_WAIT;
	    }
	    ...
	    if(rc != 0) {
	        ...
	        if (DEBUG_FLAG) {
	        	printf("readChildProcessData: child process exit successfully\n");
	        }

	        if (outRedirect && (readAndWriteData(outFd, fdout, outRedirect) == IBM_ISERIES_REDIRECT_ERR_WRITE)) {
	        	return IBM_ISERIES_REDIRECT_ERR_WRITE;
	        }

	        if (errRedirect && (readAndWriteData(errFd, fderr, errRedirect) == IBM_ISERIES_REDIRECT_ERR_WRITE) ) {
	        	return IBM_ISERIES_REDIRECT_ERR_WRITE;
	        }
	    }
    }
 ...
 * stdout_pfd: file descriptor of PIPE used by parent process to read output of child process related with STDOUT
 * stderr_pfd: file descriptor of PIPE used by parent process to read output of child process related with STDERR
 * fdout:      file descriptor used by parent process to write the output of child process
 * fderr:      file descriptor used by parent process to write the output of child process
...
static int SpawnProcess(JNIEnv *env, int stdin_fd, int stdout_fd,               //IBM-iseries_bringup
                        int stderr_fd, int stdout_pfd, int stderr_pfd,
                        int fdout, int fderr,
                        char *jobname, char *chdir_path,                       //IBM-iseries_bringup
...
{                                                                               //IBM-iseries_bringup
    ...
    int outRedirect = IBM_ISERIES_NON_REDIRECT;
    int errRedirect = IBM_ISERIES_NON_REDIRECT;
    ...
    margs->stdin_fd  = getIleFD(env, stdin_fd);                                 //IBM-iseries_bringup
    margs->stdout_fd = getIleFD(env, stdout_fd);                                //IBM-iseries_bringup
    margs->stderr_fd = getIleFD(env, stderr_fd);                                //IBM-iseries_bringup
    if (DEBUG_FLAG) {                                                           //IBM-iseries_bringup
        printf("stdin_fd=%d,   stdout_fd=%d,   stderr_fd=%d,   stdout_pfd=%d,   stderr_pfd=%d,   fdout=%d, fderr=%d, outRedirect=%d, errRedirect=%d\n",            //IBM-iseries_bringup
                stdin_fd, stdout_fd, stderr_fd, stdout_pfd, stderr_pfd, fdout, fderr, outRedirect, errRedirect);                           //IBM-iseries_bringup
    ...
    if ( fdout != -1 ) {
	    if (fdout == STDOUT_FILENO) {
		    outRedirect = IBM_ISERIES_REDIRECT_STANDARD_OUT;
	    }
	    else {
		    outRedirect = IBM_ISERIES_REDIRECT_OUT_TO_FILE;
	    }
    }

    if ( fderr != -1 ) {
	    if ( fderr == STDERR_FILENO) {
		    errRedirect = IBM_ISERIES_REDIRECT_STANDARD_ERR;
	    }
	    else {
		    errRedirect = IBM_ISERIES_REDIRECT_ERR_TO_FILE;
	    }
    }

    if ( outRedirect ) {
        setFileNonblock(stdout_pfd);
	}
    if ( errRedirect) {
		setFileNonblock(stderr_pfd);
	}
    ...
    if (pid < 0) { /* It is an errno returned from ILE */                       //IBM-iseries_bringup
    	...
    	if ( (outRedirect != IBM_ISERIES_NON_REDIRECT) || (errRedirect != IBM_ISERIES_NON_REDIRECT) ) {
    		/*
    		 * the write end pipe in child process will be closed after child process exit.
    		 * if don't close the write end in the parent process, select() will wait for the write end forever which is not closed in parent process
    		 */
    		if (outRedirect != IBM_ISERIES_NON_REDIRECT) {
    			close(stdout_fd);
    		}

    		if (errRedirect != IBM_ISERIES_NON_REDIRECT) {
    			close(stderr_fd);
    		}

    	    int rc = readChildProcessData(stdout_pfd, stderr_pfd, fdout, fderr, outRedirect, errRedirect, pid);
    	    if (rc == IBM_ISERIES_REDIRECT_ERR_SELECT) {
    	    	throwIOException(env, errno, "select failed when read output of child process");
    	    	return -1;
    	    }

    	    if (rc == IBM_ISERIES_REDIRECT_ERR_WAIT) {
    	    	Throw_InternalError(env, -1*pid, errno, "wait failed when read output of child process");
    	    	return -1;
    	    }

    	    if (rc == IBM_ISERIES_REDIRECT_ERR_WRITE) {
    	    	throwIOException(env, errno, "write failed when read output of child process");
    	    	return -1;
    	    }

    	    if (DEBUG_FLAG) {
    	    	printf("SpawnProcess: readChildProcessData return value: %d\n", rc);
    	    }
    	}
...
static char** getOs400envv(JNIEnv *env, const char* penvBlock, jint envc, int* inherit, int* malloc_index) { //IBM-iseries_bringup
    ...
    /* User entered + I5 OS specific envvars                                    //IBM-iseries_bringup
       + current envvars (if applicable) + 1 for null terminator */             //IBM-iseries_bringup
    int malloc_size = envc + max_forced_envs + environCount + 1;                //IBM-iseries_bringup
                                                                                //IBM-iseries_bringup
...
startOs400Child(JNIEnv *env, ChildStuff *c, int inherit, int malloc_index) {    //IBM-iseries_bringup
   ...
   if (DEBUG_FLAG) {                                                            //IBM-iseries_bringup
	 ...
	 int fdin  = c->in[0];                                                  //IBM-iseries_bringup
   int fdout = c->out[1];                                                       //IBM-iseries_bringup
   int fderr = c->err[1];                                                       //IBM-iseries_bringup
...
*/                                                                              //IBM-iseries_bringup
   if (c->redirectErrorStream) {                                                //IBM-iseries_bringup
        if ((closeSafely(c->err[1]) == -1) ||                                   //IBM-iseries_bringup
            (restartableDup2(c->out[1], c->err[1]) == -1))                      //IBM-iseries_bringup
           return -1;                                                           //IBM-iseries_bringup
   }                                                                            //IBM-iseries_bringup
                                                                                //IBM-iseries_bringup
   ...
   /* This is the default */                                                    //IBM-iseries_bringup
	 if (Spawn_In_ILE(env)) {                                               //IBM-iseries_bringup
	    ...
	    resultPid = SpawnProcess(env, fdin, fdout, fderr, c->out[0], c->err[0], c->fds[1], c->fds[2], jobname, (char*)c-> pdir, //IBM-iseries_bringup
